...
3. Run `voicer --vtt narration.vtt --voice Greg --output build/greg-track.wav --format pcm_44100 --s3-bucket audio-fragments` to render Greg's take. Repeat with `--voice Nichalia` to produce the alternate read, writing to a different output file.
...
7. On re-runs, voicer will read cached segments either from the local temporary directory or from the `audio-fragments` S3 bucket, accelerating subsequent renders.
