...
This is now an optional manual pipeline. The learner-facing e-learning site uses topic HTML plus compressed `audio.mp3` delivery files and `subtitles.vtt`; normal cron automation runs `scripts/build_audio.sh` to create `audio.wav` masters and does not build or publish `final.mp4`.
