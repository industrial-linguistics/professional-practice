...
- Audio generation is run from `professionalpractice@raksasa` with `scripts/audio_generation_cron.sh`. The wrapper takes a lock, pulls the latest `main`, sources `/home/professionalpractice/.config/professional-practice.env`, passes `/home/professionalpractice/.cache/professional-practice-audio-tmp/voicer-audio` as the persistent local audio-segment cache, and invokes `scripts/audio_generation_worker.py` for a bounded number of valid topics per run. The worker uses `scripts/build_audio.sh`, so normal automation generates `subtitles.vtt` and `audio.wav` audio masters only; it does not render slide PNGs or assemble `final.mp4`. The e-learning build transcodes available masters to `audio.mp3` for browser delivery before publishing. The worker defaults to a conservative budget guard: `10,000` estimated Professional Practice characters per UTC day, `250,000` per UTC month, and a `500,000` shared ElevenLabs account-period cap so rollover credits are treated as reserve.
- When a cron run builds new audio, it rebuilds `output/site` with `scripts/build_elearning.py` and publishes the static site to `merah` with `scripts/deploy_website.sh`. Missing MP4s are intentionally not shown as pending on the learner-facing course surface. `scripts/publish_videos.sh` remains available for manual MP4 publication if that output is needed later.
