...
- Audio generation is run from `professionalpractice@raksasa` with `scripts/audio_generation_cron.sh`. The wrapper takes a lock, pulls the latest `main`, sources `/home/professionalpractice/.config/professional-practice.env`, passes `/home/professionalpractice/.cache/professional-practice-audio-tmp/voicer-audio` as the persistent local audio-segment cache, and invokes `scripts/audio_generation_worker.py` for a bounded number of valid topics per run.
