...
- Audio generation is run from `professionalpractice@raksasa` with `scripts/audio_generation_cron.sh`. The wrapper takes a lock, pulls the latest `main`, sources `/home/professionalpractice/.config/professional-practice.env`, passes `/home/professionalpractice/.cache/professional-practice-audio-tmp/voicer-audio` as the persistent local audio-segment cache, and invokes `scripts/audio_generation_worker.py` for a bounded number of valid topics per run. The worker defaults to a conservative budget guard: `10,000` estimated Professional Practice characters per UTC day, `250,000` per UTC month, and a `500,000` shared ElevenLabs account-period cap so rollover credits are treated as reserve.
- Generated lesson MP4s are published from the `raksasa` checkout to `merah` with `scripts/publish_videos.sh`. The script mirrors generated `content/part-*/*/final.mp4` files into `/var/www/vhosts/professional-practice.industrial-linguistics.com/htdocs/videos/` and writes a public video index.
