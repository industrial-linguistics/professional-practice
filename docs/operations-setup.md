...
- Audio generation is run from `professionalpractice@raksasa` with `scripts/audio_generation_cron.sh`. The wrapper takes a lock, pulls the latest `main`, sources `/home/professionalpractice/.config/professional-practice.env`, uses `/home/professionalpractice/.cache/professional-practice-audio-tmp` for persistent local audio segments, and invokes `scripts/audio_generation_worker.py` for a bounded number of valid topics per run.
