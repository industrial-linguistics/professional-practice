There's a phrase inside software vendors for a salesperson promising features that don't exist yet: "selling the roadmap's roadmap." It always starts innocently. A prospect asks whether the product can do X; the rep, keen to keep the deal alive, says "absolutely — that's coming next quarter." Without anyone from the product team in the room, nobody corrects the record, and the promise becomes an expensive IOU that engineering discovers only after the contract is signed. Product team alignment is the set of habits that stops this happening, and it shows why cross-functional communication is a technical skill rather than an optional soft one.
