...
A log is the running commentary an application writes about itself: each entry carries a timestamp and a severity level — DEBUG for developer chatter, INFO for normal events, WARN for things that look off, ERROR for things that definitely are. One engineer in the book's fictional cast describes logs as the place your application goes to confess its sins, and the confession metaphor is apt: the entries are honest, unflattering, and only useful if someone listens.
