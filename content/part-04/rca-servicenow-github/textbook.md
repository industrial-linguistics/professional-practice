...
Recall the ITIL distinction from Part 2: an *incident* is the fire (restore service now) while a *problem* is the underlying cause that keeps lighting fires. The problem record is where root cause analysis findings belong, created once the investigation has something to say.
