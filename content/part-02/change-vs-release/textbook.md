...
Finally, the team **updates the CMDB and closes the record**. This step is easy to omit in a simulation and costly to omit in production. The configuration management database (covered in its own topic later in this part) is only trustworthy if every change updates it. A change process that ends at "it works" rather than "it works and the records reflect it" is slowly poisoning every future troubleshooting session.
