...
First, IT exists to deliver *services*, not systems. Nobody at Coralline wants a database; they want customers to be able to pay for cushions at eleven o'clock at night. The database is a means. Framing your work as service delivery changes what you measure and what you apologise for.
...
## Why employers keep asking for it
...
It also compounds. Incident management, change enablement and problem management are foundations for much of enterprise operations, and the DevOps and reliability engineering material later in this book assumes them constantly. Learn the vocabulary now and every subsequent topic gets easier; skip it and you'll be reverse-engineering it from context.

There is a formal credential if you want one. PeopleCert can change the ITIL Foundation exam format and registration rules, so check its current material directly. The professional point is steadier: a credential can signal familiarity with the framework, but it does not replace operational judgement. This book covers the working vocabulary and decisions that the credential is intended to support. [@peoplecert-itil4]
...
That is the practical case for ITIL 4. It provides a toolkit and, more usefully, the *names* of the tools, so that when something breaks you do not have to guess which wrench fits. You do not need to memorise every practice at once. You need the map: to know that structured responses exist for incidents, requests, changes and problems, and to recognise which one a given situation calls for. The rest of this part fills in those tools one at a time, starting with the value chain that connects all of them and ending with the careers built on top.
